...
function checkCashRegister(price, cash, cid) {
    ...
    if ( changeDue < moneyInRegister ) answer.status = 'OPEN';
    if ( changeDue === moneyInRegister ) answer.status = 'CLOSED';
    ...
    answer.change = register.reduce((acc, elem) => {
      partial = Math.min(elem[1], Math.floor(changeDue / denoms[elem[0]]) * denoms[elem[0]]);
      if ( partial > 0) {
        changeDue -= partial;
    ...
    if ( changeDue > moneyInRegister || changeDue > 0) {
